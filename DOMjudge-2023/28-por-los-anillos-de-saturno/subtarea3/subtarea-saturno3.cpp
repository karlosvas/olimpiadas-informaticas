...
void solve() {
  ...
  int costeE = 0;
  for (int n = 1; n < N + 1; ++n) {
    // Timepo que tarda un alienígena en recorrer un grado de distancia angular,
    // en el i-esimo anillo.
    int T;
    cin >> T;
    peso_anillos[n] = T;
  }
  ...
  // Distancia conocida recorrida.(w), coste del recorrido.(cost), distancia
  // desde la distancia conicida hasta la distancia almacenada en el anillo(u)
