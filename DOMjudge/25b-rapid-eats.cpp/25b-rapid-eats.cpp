...
vector<int> ant; // Para mostrar los vertices anteriore con menor coste por los que pasa.
int N, C;

// Mostrar camino de forma recursiva he invertida.
void camino(int v, int P){
    // Si el vértice es el origen, se muestra y se termina la recursividad.
    if(v == P) {
        cout<<P;
        return;
    }else{
        // Si no, se muestra el camino del vértice anterior al actual.
        camino(ant[v], P);
        cout<<" -> "<<v;    
    }
}
...
int Dijsktra(int P, int Q){
    ...
    while(!pq.empty()){
        ...
        // Minutos de adyacentes(ma), vértice adyacente(w).
        for(auto[ma, w] : g[v]){
            ...
            if(dist[v] + ma < dist[w]){
                ...
                dist[w] = dist[v] + ma;
                // Guarda el vértice anterior por el que se ha pasado para llegar a w.
                ant[w] = v;
                pq.push({dist[w], w});
            }
        }
...
bool solve(){
    ...
    for(int k = 0; k < K; ++k){
        ...
        if(sol == INF) cout<<"NO LLEGA\n";
            ...
            // Si ha llegado, muestra el tiempo que ha tardado y el camino que ha seguido.
            cout<<sol<<": ";
            camino(Q, P);
            cout<<"\n";
        }
